...
# Tested with good old PL2303HXA
__gpios = [ gpio(0x01, 0x10, 0x01, 0x40), gpio(0x01, 0x20, 0x01, 0x80)]

def pin_to_gpio(pin):
    if pin > len(__gpios):
        raise Exception(f"invalid gpio number {pin}")
    return __gpios[pin]

def open(driver, device):
    handle = pygpiotools.tty_to_usbdev(device, VID, PID)
    return handle

def __read_pl2303_reg(device, reg_num):
...
def __write_pl2303_reg(device, reg_num, value):
